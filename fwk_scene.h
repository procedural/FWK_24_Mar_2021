...
void object_update(object_t *obj) {
    quat p = eulerq(vec3(obj->pivot.y,obj->pivot.x,obj->pivot.z));
    quat e = eulerq(vec3(obj->euler.y,obj->euler.x,obj->euler.z));
    compose44(obj->transform, obj->pos, mulq(p, e), obj->sca);
//    compose44(obj->transform, obj->pos, obj->rot, obj->sca);
}
...
void object_rotate(object_t *obj, vec3 euler) {
    quat p = eulerq(vec3(obj->pivot.y,obj->pivot.x,obj->pivot.z));
    quat e = eulerq(vec3(euler.y,euler.x,euler.z));
    obj->rot = mulq(p,e);
    obj->euler = euler;
    object_update(obj);
}
...
scene_t* scene_push() {
    ...
    s->program = shader(vertex_shader_332_32, fragment_shader_32_4, "att_position,att_normal,att_texcoord", "fragcolor");
...
void scene_render(int flags) {
    ...
    if(flags & SCENE_BACKGROUND) {
        ...
        float mvp[16]; multiply44x2(mvp, cam->proj, cam->view);
        skybox_push_state(&last_scene->skybox, mvp);
...
//  glUseProgram(last_scene->program);
    ...
    if( flags & SCENE_FOREGROUND ) {
        for(unsigned j = 0, obj_count = scene_count(); j < obj_count; ++j ) {
            ...
            float mvp[16]; multiply44x3(mvp, cam->proj, cam->view, obj->transform);
            model_render(*model, mvp);
#endif
        }
        glBindVertexArray(0);
    }
