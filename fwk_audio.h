...
static audio_queue_t audio_queues[1024] = {0};
static int audio_queue_rd = 0, audio_queue_wr = 1024/2, audio_queue_voice = -1;
...
if(front->data)
            ...
            if( front->avail <= 0 ) {
                audio_queue_rd = (audio_queue_rd + 1) % 1024;
            }
...
int audio_queue( const void *samples, int num_samples, int flags ) {
    ...
    int bytes = num_samples * channels * bytes_per_sample;
    assert( channels == 2 );
    ...
    ONCE thread_mutex_init(&queue_mutex);
    thread_mutex_lock(&queue_mutex);
        audio_queue_t *aq = &audio_queues[audio_queue_wr = (audio_queue_wr+1) % 1024];
        aq->cursor = 0;
        aq->avail = bytes;
